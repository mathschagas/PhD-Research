...
def quote(component_type, lat1, lon1, lat2, lon2, component_id):
    if component_type not in quote_functions:
        return jsonify(
            status="ERROR",
            message="Component type not found."
        )

    speed = generate_fixed_values(component_id, QUOTE_DATA[component_type]['speed_min'], QUOTE_DATA[component_type]['speed_max'])
    cost_per_km = generate_fixed_values(component_id, QUOTE_DATA[component_type]['cost_per_km_min'], QUOTE_DATA[component_type]['cost_per_km_max'])
    distance = calculate_haversine_distance(lat1, lon1, lat2, lon2)
    distance_to_package = generate_fixed_values(component_id, QUOTE_DATA[component_type]['distance_to_package_min'], QUOTE_DATA[component_type]['distance_to_package_max'])
    time_hours = (distance + distance_to_package) / speed
    time_to_deliver = round(time_hours * 60, 2)
    price = round(distance * cost_per_km, 2)
    ...
    return jsonify(
        ...
        cbr={
            "time_to_deliver": time_to_deliver,
            "price": price,
            "safe_to_rain": generate_fixed_values(component_id, QUOTE_DATA[component_type]['safe_to_rain_min'], QUOTE_DATA[component_type]['safe_to_rain_max']),
            "secure_container": generate_fixed_values(component_id, QUOTE_DATA[component_type]['secure_container_min'], QUOTE_DATA[component_type]['secure_container_max'])
        }
    )
